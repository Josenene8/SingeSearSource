...
class PlayState extends MusicBeatState
{
	...
	var santa:FlxSprite;
	var Neonbarflamed:FlxSprite;
	var molotovbar:FlxSprite;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'idk-love':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/idk-love/idk-loveDialogue'));
				hasDialogue = true;
			case 'heartburn':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/heartburn/heartburnDialogue'));
				hasDialogue = true;
				endDialogue = CoolUtil.coolTextFile(Paths.txt('data/heartburn/heartburnEnd'));
				hasEnding = true;
			case 'seared':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/seared/searedDialogue'));
				hasDialogue = true;
				endDialogue = CoolUtil.coolTextFile(Paths.txt('data/seared/searedEnd'));
				hasEnding = true;
			case 'molotov':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/molotov/molotovDialogue'));
				hasDialogue = true;
		}
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				...
				case 'neonbar':
					...
					var bg:FlxSprite = new FlxSprite(-200, -80);
					bg.loadGraphic(Paths.image("Singe/Neonbarbehind"));
					add(bg);
					bg.updateHitbox();
					...
					var bg:FlxSprite = new FlxSprite(-200, -80);
					bg.loadGraphic(Paths.image("Singe/Neonbarfront"));
					add(bg);
					bg.updateHitbox();
				...
				case 'wireframebar':
					{
						curStage = 'wireframebar';
						defaultCamZoom = 0.85;

					var bg:FlxSprite = new FlxSprite(-200, -80);
					bg.loadGraphic(Paths.image("Singe/Neonbar_WF"));
					add(bg);
					bg.updateHitbox();
					bg.setGraphicSize(Std.int(bg.width * 1.08));
					bg.antialiasing = true;
				...
				case 'Neonbarflamed':
					{
						curStage = 'Neonbarflamed';
						defaultCamZoom = 0.80;

						Neonbarflamed = new FlxSprite(-200, -80);
						Neonbarflamed.frames = Paths.getSparrowAtlas('Singe/Neonbarflamed');
						Neonbarflamed.animation.addByPrefix('idle', 'Neonbarflamed', 24, false);
						Neonbarflamed.setGraphicSize(Std.int(Neonbarflamed.width * 1.10));
						if(FlxG.save.data.antialiasing)
							{
								Neonbarflamed.antialiasing = true;
							}
				...
				case 'molotovbar':
					{
						...
						molotovbar = new FlxSprite(-200, -80);
						molotovbar.frames = Paths.getSparrowAtlas('Singe/molotovbar');
						molotovbar.animation.addByPrefix('idle', 'molotovbar', 24, false);
						molotovbar.setGraphicSize(Std.int(molotovbar.width * 1.06));
						if(FlxG.save.data.antialiasing)
							{
								molotovbar.antialiasing = true;
							}
						if (FlxG.save.data.distractions)
						{
							add(molotovbar);
		...
		switch (SONG.player2)
		{
			...
			case 'sear-wf':
				dad.x += 50;
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y + 150);
				if (FlxG.save.data.distractions)
				{
					// trailArea.scrollFactor.set();
					if (!PlayStateChangeables.Optimize)
		...
		endshit.finishThing = startCountdown;
	...
	function endCutscene(dialogueBox:DialogueBox){
        	...
        	trace("endCutscene");
        	var black:FlxSprite = new FlxSprite(-256, -256).makeGraphic(FlxG.width * 6, FlxG.height * 6, FlxColor.BLACK);
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			{
				if (dialogueBox != null && (hasDialogue == true))
				{
					inCutscene = true;
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (dad.curCharacter == 'sear') {
       					FlxG.camera.shake(0.01, 0.01);
					}

					if (dad.curCharacter == 'sear-wf') {
       					FlxG.camera.shake(0.01, 0.01);
					}
	...
	function endSong():Void
	{
		...
		if (offsetTesting)
		{
			FlxG.sound.playMusic(Paths.music('singeMenu'));
			offsetTesting = false;
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					switch(SONG.song.toLowerCase())
                        {
                            case 'seared':
                                LoadingState.loadAndSwitchState(new VideoState("assets/videos/Searcut.webm", new PlayState()));
                            default:
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'Neonbarflamed':
				if (FlxG.save.data.distractions)
				{
					Neonbarflamed.animation.play('idle', true);
				}
			case 'molotovbar':
				if (FlxG.save.data.distractions)
				{
					molotovbar.animation.play('idle', true);
				}
